...
public class CountOfProductsInCartTest_withDataprovider extends BaseTest {
    ...
    @TestName(name = "Count of Items in Cart")
    public void viewCart_Update_Test(ProductTypes type, Class page, String itemEntity) throws Exception {
        ...
        //test steps
        home.open();
        //adding few items with first variation of options
        ...
        //checking if first-variation products were counted
        countOfGoodsFromCartIcon = product.header.getCountOfGoodsFromMiniCartIcon();
        countOfGoodsInMiniCart = product.header.getCountOfGoodsInMiniCart();
        Assert.assertTrue(countOfGoodsInMiniCart == countOfGoodsFromCartIcon, "First time. Count of added to cart items equal to count from cart icon");

        //adding few items with second variation of options
        ProductSync.check(type);
        for(int i = 0; i < 2; i++) {
            product.open();
            product.isProductInStock()
        ...
        countOfGoodsFromCartIcon = product.header.getCountOfGoodsFromMiniCartIcon();
        countOfGoodsInMiniCart = product.header.getCountOfGoodsInMiniCart();
        Assert.assertTrue(countOfGoodsInMiniCart == countOfGoodsFromCartIcon, "Second time. Count of added to cart items equal to count from cart icon");
